...
def Retrieve_last_trigger_params(location):
    ...
    if db != []:
        try:
            cursor = db.cursor()
            query = "SELECT * FROM trigger_v where external_trigger_type_simulation=1  ORDER BY ID_external_trigger DESC LIMIT 1"
            ...
            print("lost connection to Mysql server during the query")
            d = {}
...
def Retrieve_newer_trigger_params(location,ID_external_trigger_old):
    ...
    if db != []:
        try:
            cursor = db.cursor()
            query = "SELECT * FROM trigger_v WHERE external_trigger_type_simulation=1  and ID_external_trigger > " + str(ID_external_trigger_old) + " ORDER BY ID_external_trigger "
            cursor.execute(query)
            results = cursor.fetchall()
            ...
            if not results:
                ...
                for n in range(len(results)):
                    ...
                    external_trigger_RA_center.append(results[n][6])
                    external_trigger_dec_center.append(results[n][7])
                    external_trigger_localisation_error.append(results[n][8])
                    ID_external_trigger_type.append(results[n][18])
                    external_trigger_type_name.append(results[n][19])
                    ID_external_trigger_telescope.append(results[n][33])
...
#__________________________________
def trigger_type_classification(trigger_type,ID_external_trigger_telescope,alert_message_type):
    ...
    if alert_message_type[n] == 'Initial':
        # print('Initial')
        mark2 = 1000
    elif alert_message_type[n] == 'Update':
        # print('Update')
        mark2 = 2000
    else:
        mark2 = 0          

    return(mark1,mark2,mark5) 
...
def Telescope_Obs_strategy_match(mark,location):
    ...
    if mark in [11111,11121,12111,12121,11112,11122,12112,12122]:
        ...
        obs_type_id = [13,14]  
        match = 1
    elif mark in [21111,21121,21112,21122,22111,22121,22112,22122]:
        obs_type_id = [14]  
        match = 1
    elif mark in [21211,21221,21212,21222,22211,22221,22212,22222]:
        obs_type_id = [13]  
        match = 1
